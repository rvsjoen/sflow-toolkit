...
class MyFuncs:
	...
	def get_cntr_index(self, agent, start, end, fields, index): 
		global i 
		logfile.write(time.asctime()+" REQUEST [%u] get_cntr_index(%s, %s, %s, %s, %u) \n"%(i, agent, start, end, fields, index))
	...
	def get_flow_index(self, agent, start, end, fields, index):
		global i
		logfile.write(time.asctime()+" REQUEST [%u] get_flow_index(%s, %s, %s, %s, %u)\n"%(i, agent, start, end, fields, index));
	...
	def get_conv_index(self, agent, start, end, index):
		global i
		logfile.write(time.asctime()+" REQUEST [%u] get_conv_index(%s, %s, %s, %u)\n"%(i, agent, start, end, index));
